...
export const first_person_camera = (() => {
  ...
  class FirstPersonCamera extends entity.Component {
    ...
    updateRotation_(timeElapsedS) {
      ...
      // Use mouseXDelta and mouseYDelta directly, scaled by the sensitivity factor
      const xh = -input.current_.mouseXDelta * sensitivity;
      const yh = -input.current_.mouseYDelta * sensitivity;
  
      // Reset the mouse deltas
      input.current_.mouseXDelta = 0;
      input.current_.mouseYDelta = 0;
  
      // Horizontal rotation
      this.phi_ += xh * this.phiSpeed_;
  
      // Vertical rotation (theta)
      this.theta_ += yh * this.thetaSpeed_;
  
      // Clamp the vertical rotation (theta) to prevent over-rotation
      const MAX_VERTICAL_ANGLE = Math.PI / 2; // 90 degrees in radians
      this.theta_ = Math.max(-MAX_VERTICAL_ANGLE, Math.min(MAX_VERTICAL_ANGLE, this.theta_));
